# Tài liệu Test API Authentication với Postman
...
- Đặt tên: `Expense Tracker API - Auth`
...
### 5. Lưu Token vào Environment Variable (Tùy chọn)
- Sau khi đăng nhập thành công, copy token
- Thêm biến vào Environment: `auth_token` = `<token đã copy>`
- Sử dụng `{{auth_token}}` trong các request khác
...
- Token từ đăng nhập sẽ được sử dụng cho các API User (xem file `POSTMAN_API_TEST_USER.md`)
